...
public class ColorSliderRatio extends View {
    ...
    public ColorSliderRatio(Context context) {
        this(context, null);
    }

    public ColorSliderRatio(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public ColorSliderRatio(Context context, AttributeSet attributeSet, int i) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mSelectorPaint.setColor(ContextCompat.getColor(context, android.R.color.darker_gray)); // Default selector color
        mSelectorPaint.setStrokeWidth(2.0f);

        // Set touch listener
        setOnTouchListener(new ColorSliderTouchListener());

        int selectorColor = 0; // Default selector color
        if (attrs != null) {
            // Obtain styled attributes from XML
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ColorSlider, 0, 0);

            try {
                // Extract color and resource attributes
                selectorColor = typedArray.getColor(3, 0);
                int colorsArrayResId = typedArray.getResourceId(0, 0);
                int colorStringsResId = typedArray.getResourceId(2, 0);

                if (colorsArrayResId != 0) {
                    // If a color array is defined, load it
                    int[] colorsArray = getResources().getIntArray(colorsArrayResId);
                    if (colorsArray.length > 0) {
                        mColors = new int[colorsArray.length];
                        System.arraycopy(colorsArray, 0, mColors, 0, colorsArray.length);
                    }
                } else if (colorStringsResId != 0) {
                    // If a color string array is defined, convert it to colors
                    String[] colorStrings = getResources().getStringArray(colorStringsResId);
                    if (colorStrings.length > 0) {
                        convertToColors(colorStrings);
                    }
                }

                // If no colors are defined, try generating them
                if (mColors.length == 0) {
                    int startColor = typedArray.getColor(1, 0);
                    int endColor = typedArray.getColor(5, 0);
                    int steps = typedArray.getInt(4, 21); // Default: 21 steps

                    if (startColor != 0 && endColor != 0 && steps > 0) {
                        calculateColors(startColor, endColor, steps);
                    }
                }
            } catch (Exception e) {
                Log.d("ColorSlider", "init: " + e.getLocalizedMessage());
            } finally {
                typedArray.recycle(); // Ensure we free up resources
            }
        }

        // If no colors were set, use default colors
        if (mColors.length == 0) {
            initDefaultColors();
        }

        // Initialize color rectangles for UI representation
        mColorRects = new Rect[mColors.length];
        mColorFullRects = new Rect[mColors.length];

        // Apply selector color if set
    ...
    private void calculateColors(int[] iArr, int i) {
        ColorSliderRatio colorSliderRatio = this;
        int[] iArr2 = iArr;
        ...
        colorSliderRatio.mColors = new int[i];
        int i5 = 1;
        while (i5 < length) {
            int i6 = i5 - 1;
            int i7 = iArr2[i6];
